    ...
    def centroids_find_closest(data,centroids):
        ...
        for example_index in range(num_examples):
            ...
            for centroid_index in range(num_centroids):
                distance_diff = data[example_index,:] - centroids[centroid_index,:] # 计算特征之间的差值
                distance[centroid_index] = np.sum(distance_diff**2) # 计算欧式距离
            closest_centroids_ids[example_index] = np.argmin(distance) # 找到最小的距离的索引
