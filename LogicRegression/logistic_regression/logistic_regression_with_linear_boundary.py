...
iris_types = ['SETOSA','VERSICOLOR','VIRGINICA'] # 三种鸢尾花的类别，分别是山鸢尾、变色鸢尾和维吉尼亚鸢尾
# 为了方便可视化，这里只选择了两个特征，花瓣长度和花瓣宽度

x_axis = 'petal_length' # 选择x轴和y轴，这里选择花瓣长度和花瓣宽度
y_axis = 'petal_width'

for iris_type in iris_types:
    plt.scatter(data[x_axis][data['class']==iris_type], # data['class']==iris_type是一个布尔数组，data[x_axis][data['class']==iris_type]是一个Series
                data[y_axis][data['class']==iris_type],
                label = iris_type # 为每个类别的散点图添加标签
                )
plt.show()

num_examples = data.shape[0]
x_train = data[[x_axis,y_axis]].values.reshape((num_examples,2)) # 选择两个特征作为输入
y_train = data['class'].values.reshape((num_examples,1)) # 选择类别作为输出
...
samples= 150 
X = np.linspace(x_min,x_max,samples) # np.linspace生成等差数列
Y = np.linspace(y_min,y_max,samples)

Z_SETOSA = np.zeros((samples,samples)) # 生成一个samples*samples的全零矩阵，表示三种鸢尾花的预测结果
# 行表示x轴的取值，列表示y轴的取值
